...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b px-4 md:px-6 py-2 bg-white">
          ...
          <nav className="flex items-center space-x-6 text-sm font-semibold">
            {
              navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  to={link.path} 
                  className={`hover:text-violet-600 hover:border-b-2 hover:border-violet-600 transition-all duration-100 ease-in-out ${activeLink === link.path ? 'text-violet-600 border-b-2 border-violet-600' : ''}`}
                >
                  {link.name}
                </Link>
              ))
